...
class Person {
  constructor(name, age, point) {
    // name, age는 객체를 초기화 할 때 생성할 멤버 변수
    this.name = name; // property = parameter
    this.age = age;
    this.point = point;
  }
  showMe() {
    // method(operation, behavior이라고도 함)
...
class Man extends Person {
  // Man --> Person 처럼 그림으로 표현함. 부모와 자식관계.sub class --> super class . "부모 + @"
  ...
  showMe() {
    // 같은 이름의 함수 생성시 오버라이딩 발생. "오버라이딩 : 부모에게 받은 메소드를 재정의하는 것.(그대로 쓰지 않고 내가 필요한 내용을 넣어서 다시 만듦)"
    return (
      "Name: " +
      this.name +
      " Age: " +
